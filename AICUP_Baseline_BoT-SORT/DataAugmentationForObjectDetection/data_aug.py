...
txt_trainPath = '/home/wish/pro/AICUP/MCMOT/AICUP_Baseline_BoT-SORT/datasets/AI_CUP_MCMOT_dataset/yolo/train/labels/'
img_trainPath = '/home/wish/pro/AICUP/MCMOT/AICUP_Baseline_BoT-SORT/datasets/AI_CUP_MCMOT_dataset/yolo/train/images/'
txt_valPath = '/home/wish/pro/AICUP/MCMOT/AICUP_Baseline_BoT-SORT/datasets/AI_CUP_MCMOT_dataset/yolo/valid/labels/'
img_valPath = '/home/wish/pro/AICUP/MCMOT/AICUP_Baseline_BoT-SORT/datasets/AI_CUP_MCMOT_dataset/yolo/valid/images/'
...
for txtf in txt_trainList:
    ...
    if len(bboxes) > 0:
        ...
        cv2.imwrite(img_trainPath + imgf[:-4] + '_r1.jpg', img_)    # save jpg file

        bbox_pixelList = [bi for bi in bboxes_]    # l: left upper x, left upper y, w, h, cls_id
        with open(txt_trainPath + txtf[:-4] + '_r1.txt', 'w') as f:    # save txt file
            for bbox_pi in bbox_pixelList:
                lux, luy, rbx, rby, cls_id = bbox_pi
                w, h = rbx - lux, rby - luy
                yolox, yoloy, yolow, yoloh = (lux + w/2) / imx, (luy + h/2) / imy, w / imx, h / imy
        ...
        # Rotate -1
        img_, bboxes_ =RandomRotate((-1, -1))(img.copy(), bboxes.copy())
        cv2.imwrite(img_trainPath + imgf[:-4] + '_rm1.jpg', img_)    # save jpg file

        bbox_pixelList = [bi for bi in bboxes_]    # l: left upper x, left upper y, w, h, cls_id
        with open(txt_trainPath + txtf[:-4] + '_rm1.txt', 'w') as f:    # save txt file
            for bbox_pi in bbox_pixelList:
                lux, luy, rbx, rby, cls_id = bbox_pi
                w, h = rbx - lux, rby - luy
                yolox, yoloy, yolow, yoloh = (lux + w/2) / imx, (luy + h/2) / imy, w / imx, h / imy
...
for txtf in txt_valList:
    ...
    if len(bboxes) > 0:
        ...
        # Rotate 1
        img_, bboxes_ = RandomRotate((1, 1))(img.copy(), bboxes.copy())
        cv2.imwrite(img_valPath + imgf[:-4] + '_r1.jpg', img_)    # save jpg file

        bbox_pixelList = [bi for bi in bboxes_]    # l: left upper x, left upper y, w, h, cls_id
        with open(txt_valPath + txtf[:-4] + '_r1.txt', 'w') as f:    # save txt file
            for bbox_pi in bbox_pixelList:
                lux, luy, rbx, rby, cls_id = bbox_pi
                w, h = rbx - lux, rby - luy
                yolox, yoloy, yolow, yoloh = (lux + w/2) / imx, (luy + h/2) / imy, w / imx, h / imy
        ...
        # Rotate -1
        img_, bboxes_ =RandomRotate((-1, -1))(img.copy(), bboxes.copy())
        cv2.imwrite(img_valPath + imgf[:-4] + '_rm1.jpg', img_)    # save jpg file

        bbox_pixelList = [bi for bi in bboxes_]    # l: left upper x, left upper y, w, h, cls_id
        with open(txt_valPath + txtf[:-4] + '_rm1.txt', 'w') as f:    # save txt file
            for bbox_pi in bbox_pixelList:
                lux, luy, rbx, rby, cls_id = bbox_pi
                w, h = rbx - lux, rby - luy
                yolox, yoloy, yolow, yoloh = (lux + w/2) / imx, (luy + h/2) / imy, w / imx, h / imy
